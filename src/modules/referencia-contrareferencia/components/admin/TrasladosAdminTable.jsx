...
import { trasladoService } from '../../api/adminService';
...
export function CreateEdit({ item, onClose, onActualizar }) {
  ...
  useEffect(() => {
    if (item) {
      setFormData({
        fechaTraslado: item.fechaTraslado || '',
        nomPaciente: item.nomPaciente || '',
        documento: item.documento || '',
        ingreso: item.ingreso || '',
        eps: item.eps || '',
        tipoTraslado: item.tipoTraslado || 'PROGRAMADO',
        servicio: item.servicio || '',
        destino: item.destino || '',
        ciudad: item.ciudad || '',
        autorizacion: item.autorizacion || '',
        auxiliarReferencia: item.auxiliarReferencia || '',
        auxiliarAmbulancia: item.auxiliarAmbulancia || '',
        medicamentos: Array.isArray(item.medicamentos) ? item.medicamentos.join(', ') : (item.medicamentos || ''),
        archivo: item.archivo || 'NO',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        await trasladoService.crear(payload);
      }
      onActualizar();
    } catch (err) {
      ...
      <div className="bg-white shadow-md rounded-lg p-6">
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <div>
              <label className={labelClass}>Fecha Traslado</label>
              <input type="datetime-local" value={formData.fechaTraslado} onChange={e => handleChange('fechaTraslado', e.target.value)} className={inputClass} />
            </div>
            <div>
            ...
            <div>
              ...
              <input type="text" value={formData.eps} onChange={e => handleChange('eps', e.target.value)} className={inputClass} />
            </div>
            <div>
              <label className={labelClass}>Tipo Traslado</label>
              <select value={formData.tipoTraslado} onChange={e => handleChange('tipoTraslado', e.target.value)} className={inputClass}>
                {TIPO_TRASLADO_OPTS.map(o => <option key={o} value={o}>{o}</option>)}
              </select>
            </div>
            <div>
            ...
            <div>
              ...
              <input type="text" value={formData.destino} onChange={e => handleChange('destino', e.target.value)} className={inputClass} />
            </div>
            <div>
              <label className={labelClass}>Ciudad</label>
              <input type="text" value={formData.ciudad} onChange={e => handleChange('ciudad', e.target.value)} className={inputClass} />
            </div>
            <div>
              <label className={labelClass}>Autorización</label>
              <input type="text" value={formData.autorizacion} onChange={e => handleChange('autorizacion', e.target.value)} className={inputClass} />
            </div>
            <div>
              <label className={labelClass}>Auxiliar Referencia</label>
              <input type="text" value={formData.auxiliarReferencia} onChange={e => handleChange('auxiliarReferencia', e.target.value)} className={inputClass} />
            </div>
            <div>
              <label className={labelClass}>Auxiliar Ambulancia</label>
              <input type="text" value={formData.auxiliarAmbulancia} onChange={e => handleChange('auxiliarAmbulancia', e.target.value)} className={inputClass} />
            </div>
            <div>
              <label className={labelClass}>Medicamentos</label>
              <input type="text" value={formData.medicamentos} onChange={e => handleChange('medicamentos', e.target.value)} className={inputClass} placeholder="Separados por coma" />
            </div>
            <div>
              <label className={labelClass}>Archivo</label>
              <select value={formData.archivo} onChange={e => handleChange('archivo', e.target.value)} className={inputClass}>
                {ARCHIVO_OPTS.map(o => <option key={o} value={o}>{o}</option>)}
              </select>
            </div>
            <div>
            ...
            <div className="md:col-span-2 lg:col-span-3">
              ...
              <textarea rows={3} value={formData.observaciones} onChange={e => handleChange('observaciones', e.target.value)} className={inputClass} />
            </div>
          </div>
...
export function View({ item, onClose }) {
  ...
  const medicamentosStr = Array.isArray(item.medicamentos) ? item.medicamentos.join(', ') : (item.medicamentos || '-');
  ...
  return (
    <Modal isOpen onClose={onClose}>
      <div className="bg-white shadow-md rounded-lg p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {field('ID', item.id)}
          {field('Fecha Traslado', formatDate(item.fechaTraslado))}
          {field('Nombre Paciente', item.nomPaciente)}
          {field('Documento', item.documento)}
          {field('Ingreso', item.ingreso)}
          {field('EPS', item.eps)}
          {field('Tipo Traslado', item.tipoTraslado)}
          {field('Servicio', item.servicio)}
          {field('Destino', item.destino)}
          {field('Ciudad', item.ciudad)}
          {field('Autorización', item.autorizacion)}
          {field('Auxiliar Referencia', item.auxiliarReferencia)}
          {field('Auxiliar Ambulancia', item.auxiliarAmbulancia)}
          {field('Medicamentos', medicamentosStr)}
          {field('Archivo', item.archivo)}
...
export default function TrasladosAdminTable() {
  ...
  const loadData = async () => {
    ...
    setError('');
    try {
      const response = await trasladoService.listarTodos();
      setData(response);
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('¿Estás seguro de eliminar este traslado?')) return;
    try {
      await trasladoService.eliminar(id);
      loadData();
    } catch (err) {
  ...
  return (
    <div className="bg-white shadow-md rounded-lg p-4 w-full">
      ...
      {!loading && !error && data.length === 0 && (
        <div className="text-center py-16 text-gray-500">No hay traslados registrados.</div>
      )}
      {!loading && !error && data.length > 0 && (
        <>
              ...
              <tbody>
                {paginatedData.map(t => (
                  <tr key={t.id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="px-3 py-2">{t.eps}</td>
