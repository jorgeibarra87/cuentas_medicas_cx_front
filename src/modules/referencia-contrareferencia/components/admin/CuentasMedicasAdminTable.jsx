...
import { cuentaMedicaService } from '../../api/adminService';
...
export function CreateEdit({ item, onClose, onActualizar }) {
  ...
  useEffect(() => {
    if (item) {
      setFormData({
        ...
        fechaCuenta: item.fechaCuenta || '',
        servicioEgreso: item.servicioEgreso || '',
        responsableAuditoria: item.responsableAuditoria || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEdit) {
        await cuentaMedicaService.actualizar(item.id, formData);
      } else {
        await cuentaMedicaService.crear(formData);
      }
      onActualizar();
    } catch (err) {
  ...
  return (
    <Modal onClose={onClose}>
      <div className="bg-white shadow-md rounded-lg p-6">
        <h2 className="text-xl font-bold text-gray-800 mb-4">{isEdit ? 'Editar Cuenta Médica' : 'Nueva Cuenta Médica'}</h2>
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div>
              <label className={labelClass}>Fecha Cuenta</label>
              <input type="datetime-local" value={formData.fechaCuenta} onChange={e => handleChange('fechaCuenta', e.target.value)} className={inputClass} />
            </div>
            <div>
              <label className={labelClass}>Servicio Egreso</label>
              <input type="text" value={formData.servicioEgreso} onChange={e => handleChange('servicioEgreso', e.target.value)} className={inputClass} />
            </div>
            <div>
              <label className={labelClass}>Responsable Auditoría</label>
              <input type="text" value={formData.responsableAuditoria} onChange={e => handleChange('responsableAuditoria', e.target.value)} className={inputClass} />
            </div>
            <div>
            ...
            <div>
              <label className={labelClass}>Fecha Egreso</label>
              <input type="datetime-local" value={formData.fechaEgreso} onChange={e => handleChange('fechaEgreso', e.target.value)} className={inputClass} />
            </div>
    ...
    <Modal onClose={onClose}>
      <div className="bg-white shadow-md rounded-lg p-6">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold text-gray-800">Detalle de Cuenta Médica #{item.id}</h2>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {field('ID', item.id)}
          ...
          {field('Fecha Cuenta', formatDate(item.fechaCuenta))}
          {field('Servicio Egreso', item.servicioEgreso)}
          {field('Responsable Auditoría', item.responsableAuditoria)}
          {field('Estado', item.estado)}
          {field('Fecha Egreso', formatDate(item.fechaEgreso))}
...
export default function CuentasMedicasAdminTable() {
  ...
  const loadData = async () => {
    ...
    setError('');
    try {
      const response = await cuentaMedicaService.listarTodos();
      setData(response);
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('¿Estás seguro de eliminar esta cuenta médica?')) return;
    try {
      await cuentaMedicaService.eliminar(id);
      loadData();
    } catch (err) {
  ...
  return (
    <div className="bg-white shadow-md rounded-lg p-4 w-full">
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-2xl font-bold text-gray-800">Cuentas Médicas</h1>
      ...
      {!loading && !error && data.length === 0 && (
        <div className="text-center py-16 text-gray-500">No hay cuentas médicas registradas.</div>
      )}
      {!loading && !error && data.length > 0 && (
        <>
