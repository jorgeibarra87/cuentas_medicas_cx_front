...
import { tipoSolicitudService } from '../../api/adminService';
...
function CreateEdit({ item, onClose, onActualizar }) {
  ...
  const handleChange = (e) => {
    ...
    setForm({ ...form, [name]: type === 'select-one' ? value === 'true' : value });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (item?.id) {
        await tipoSolicitudService.actualizar(item.id, form);
      } else {
        await tipoSolicitudService.crear(form);
      }
      onActualizar();
    } catch (err) {
...
function View({ item, onClose }) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="border-b border-gray-200 pb-2">
        <span className="text-xs text-gray-500 block">Activo</span>
        <span className={`inline-block px-2 py-1 rounded-full text-xs font-medium ${item.activo ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
...
export default function TipoSolicitudAdminTable() {
  ...
  const loadData = async () => {
    ...
    setError(null);
    try {
      const res = await tipoSolicitudService.listarTodos();
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('¿Está seguro de eliminar este tipo de solicitud?')) return;
    try {
  ...
  return (
    <div className="bg-white shadow rounded">
      <div className="flex justify-between items-center p-4 border-b border-gray-200">
        <h2 className="text-lg font-bold text-gray-800">Tipos de Solicitud</h2>
      ...
      {loading ? (
        ...
        <div className="text-center py-8 text-gray-500">No hay tipos de solicitud registrados</div>
      ) : (
            ...
            <tbody>
              {paginated.map((row) => (
                <tr key={row.id} className="border-b hover:bg-gray-50">
                  ...
                  <td className="px-4 py-2 text-xs">{row.descripcion}</td>
                  <td className="px-4 py-2 text-xs">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${row.activo ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
      ...
      <Modal isOpen={showCreateEdit} onClose={() => setShowCreateEdit(false)}
        title={selected?.id ? 'Editar Tipo de Solicitud' : 'Nuevo Tipo de Solicitud'} size="sm">
        <CreateEdit item={selected} onClose={() => setShowCreateEdit(false)}
          onActualizar={() => { setShowCreateEdit(false); loadData(); }} />
      </Modal>

      <Modal isOpen={showView} onClose={() => setShowView(false)} title="Detalle del Tipo de Solicitud" size="sm">
